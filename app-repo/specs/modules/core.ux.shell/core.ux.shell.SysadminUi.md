...
-   **Edit**:
    ...
    -   Block patch operations may use `null` values as tombstones inside `patch.data` to remove fields from a block's `data` payload; this is required for template override-off behavior.
-   **Delete**:
